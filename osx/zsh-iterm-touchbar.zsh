...
function _displayDefault() {
  ...
  # CURRENT_DIR
  # -----------
  pecho "\033]1337;SetKeyLabel=F3=👉 $(echo $(pwd) | awk -F/ '{print $(NF-1)"/"$(NF)}')\a"
  bindkey -s '^[OP' 'pwd \n'
  ...
  if [[ "$(git rev-parse --is-inside-git-dir 2> /dev/null)" == 'false' ]]; then
    ...
    pecho "\033]1337;SetKeyLabel=F4=🎋 $(git_current_branch)\a"
    pecho "\033]1337;SetKeyLabel=F5=$touchbarIndicators\a"
    pecho "\033]1337;SetKeyLabel=F6=✉️ push\a";
